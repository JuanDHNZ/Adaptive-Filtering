...
for rep in range(100):
    ...
    noise = 0#np.sqrt(var)*np.random.randn(samples).reshape(-1,1)
    u_train = u + noise
    d_train = d + noise
    ...
    noise = 0#np.sqrt(var)*np.random.randn(200).reshape(-1,1)
    u_test = u + noise
    d_test = d + noise
    
    # init = np.random.randint(low=0,high=len(u)-200)
    # u_test = u_test[init:init+200]
    # d_test = d_test[init:init+200]
    ...
    #    y_.append(y_pred)
        # mse_r.append(np.mean((d_test-np.array(y_pred).reshape(-1,1))**2/d_test**2))
            mse_r.append(np.sum((d_test-np.array(y_pred).reshape(-1,1))**2)/np.sum(d_test**2))
        
    mse.append(mse_r)  #Reps x Iterations
    
    mmse = np.mean(np.array(mse),axis=0)
        
    plt.figure(figsize=(15,9))
    plt.title("MSE RELATIVO - Lorenz - $\sigma$ = {} ; epsilon = {} ; Reps = {}".format(sgm,eps,rep))
    plt.yscale("log")
    plt.ylim( (1e-4,1e0))
    # plt.plot(mmse,"r-",linewidth=1)
    plt.plot(range(0,len(u_train),5),np.array(mmse).T)
    plt.ylabel("MSE")
