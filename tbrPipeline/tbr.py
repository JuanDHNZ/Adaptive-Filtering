...
class TBR(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):#prediccion
        ...
        # Input validation
        closest = numpy.argmin(euclidean_distances(X, self.X_), axis=1)
    ...
    def indices(self,fvec,flow,fhigh):     
        al = abs(fvec-flow*numpy.ones([fvec.shape[0]])).tolist()
        indl = al.index(min(al))
        ah = abs(fvec-fhigh*numpy.ones([fvec.shape[0]])).tolist()
        indh = ah.index(min(ah))
        return numpy.arange(indl,indh+1,1)