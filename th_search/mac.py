...
class MAC():
    ...
    def fit(self, X,y):
        ...
        X_ = np.transpose(X,(1,0,2)).reshape(C,-1) #Matriz con dimensiones C x NT
        ica = FastICA(n_components=C)
        S = ica.fit_transform(X_.T)  # Reconstruct signals      
    ...
    def transform(self, X):
        ...
        CORR = CORR[:,:D,D:]        
        mac = np.abs(CORR).mean(axis=2)
        
        counter = 0
        for trial in range(N):            
            ind = np.where(mac[trial] < self.th)[0]                  
            S_.append(S[trial,ind])
            A_.append(self.A[:,ind])
            Xrec.append(np.dot(A_[trial], S_[trial]))
            if(np.count_nonzero(np.dot(A_[trial], S_[trial])) < 1):
                counter += 1
        print("MAC counter = ", counter)
