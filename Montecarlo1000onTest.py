...
for _ in tqdm(range(N)):
    ...
    u = u[start:start+1000]
    d = d[start:start+1000]
    
    mse = []
    mse_ = []
      
    noise = np.sqrt(var)*np.random.randn(1000).reshape(-1,1)
    u = u + noise
    d = d + noise
    
    f = QKLMS(epsilon=eps, sigma=sgm)
    y_pred = f.evaluate(u,d)
    mse.append(((d-np.array(y_pred).reshape(-1,1))**2))
    mse_.append(((d-np.array(y_pred).reshape(-1,1))**2)/d**2)
